...
mod tests {
    ...
    use mk_core::{repository::RepositoryService, user::NewUser};
    ...
    async fn create_user(svc: &Arc<RepositoryService>) -> mk_core::user::User {
    ...
    async fn create_second_user(svc: &Arc<RepositoryService>) -> mk_core::user::User {
        use std::collections::HashSet;
        let tx = svc.repository().begin().await.unwrap();
        let user = svc
            .user_repository()
