...
pub struct FrontendConfig {
    ...
    /// e.g. 0.0.0.0
    /// Environment variable: `MAILKEEP__FRONTEND__LISTEN_IP`
    pub listen_ip: String,

    /// Port the server should listen on.
    /// e.g. 8080
    /// Environment variable: `MAILKEEP__FRONTEND__LISTEN_PORT`
    pub listen_port: u16,

    /// Base URL where the application is running.
    /// e.g. https://mailkeep.example.com
    /// Environment variable: `MAILKEEP__FRONTEND__BASE_URL`
    pub base_url: String,
}
...
pub struct OidcConfig {
    ...
    /// Environment variable: `MAILKEEP__OIDC__DISCOVERY_URL`
    pub discovery_url: Option<String>,

    /// OIDC client ID registered with the IdP.
    /// Environment variable: `MAILKEEP__OIDC__CLIENT_ID`
    pub client_id: Option<String>,

    /// OIDC client secret registered with the IdP.
    /// Environment variable: `MAILKEEP__OIDC__CLIENT_SECRET`
    pub client_secret: Option<String>,

    /// Login page button text. Defaults to "Sign in with SSO".
    /// Environment variable: `MAILKEEP__OIDC__BUTTON_LABEL`
...
impl OidcConfig {
    ...
    pub fn is_sso_available(&self) -> bool {
        ...
        if self.discovery_url.as_deref().is_none_or(str::is_empty) {
            missing.push("MAILKEEP__OIDC__DISCOVERY_URL");
        }
        if self.client_id.as_deref().is_none_or(str::is_empty) {
            missing.push("MAILKEEP__OIDC__CLIENT_ID");
        }
        if self.client_secret.as_deref().is_none_or(str::is_empty) {
            missing.push("MAILKEEP__OIDC__CLIENT_SECRET");
        }
...
#[cfg(feature = "web")]
pub mod web {
    use crate::MailKeepFrontend;

    pub fn launch_web_frontend() {
        dioxus::launch(MailKeepFrontend);
    }
}
