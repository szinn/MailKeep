...
use mk_core::{
    ...
    imap::{FolderConfig, ImapConnectionParams, ImapCredentials, ImapPort, ImapServerConfig, SyncState, TlsMode},
...
/// tears greenmail down. Mirrors `greenmail.rs::Greenmail::start` but exposes
/// the raw connection coordinates so tests can build both the system-under-test
/// adapter and an independent control client.
struct Greenmail {
...
impl Greenmail {
    ...
    fn server(&self) -> ImapServerConfig {
        ImapServerConfig {
...
/// shared by both the system-under-test adapter and the control client. Mirrors
/// `greenmail.rs::insecure_adapter`.
fn insecure_tls() -> Arc<ClientConfig> {
...
/// Entirely independent of the system-under-test adapter (different
...
impl Control {
    async fn connect(gm: &Greenmail) -> Result<Self, Error> {
    ...
    /// `mailbox`.
    async fn append(&mut self, mailbox: &str, subject: &str) -> Result<(), Error> {
    ...
    async fn ensure_mailbox(&mut self, mailbox: &str) {
        let _ = self.session.create(mailbox).await;
    }

    /// SELECT a mailbox and return its server UIDVALIDITY.
    async fn select_uidvalidity(&mut self, mailbox: &str) -> Result<u32, Error> {
        let mb = self
            .session
            .select(mailbox)
            .await
...
async fn list_messages(repos: &Arc<RepositoryService>, account_id: AccountId) -> Vec<Message> {
...
async fn wait_for_messages(repos: &Arc<RepositoryService>, account_id: AccountId, n: usize, timeout: Duration) -> Vec<Message> {
...
async fn assert_ciphertext_on_disk(ctx: &TestContext, account_id: AccountId, msg: &Message) {
...
/// TEST-ONLY certificate verifier accepting any server certificate (greenmail
/// ships a self-signed cert). Quarantined in a child module; mirrors
/// `greenmail.rs::danger::NoVerify`.
mod danger {
    use rustls::{
